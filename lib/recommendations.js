...
const toArabicTitle = (title = "") => {
  if (title.toLowerCase().startsWith("renew ")) {
    const doc = title.slice(6);
    return `تجديد ${toArabicDoc(doc)}`;
  }
  if (title.toLowerCase().startsWith("pay violation")) {
    return "سداد مخالفة";
  }
  if (title.toLowerCase().startsWith("plan upcoming travel")) {
...
const toArabicReason = (detail = "") => {
  ...
  if (/days left/i.test(detail)) return `متبقي: ${detail}`;
  if (/expiry/i.test(detail)) return `بيانات الانتهاء: ${detail}`;
